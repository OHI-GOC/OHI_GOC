...
# ![](figs/sense-of-place.svg){width=8% height=8%} DRAFT: Sense of Place EWG 3.2 {#ewg32-sp}
...
(NOTE: draft in progress, will be updated.)
...
We are not hosting a separate full-polygon map for every LSP type on this page, at least not as of now. Embedding several detailed Leaflet layers (especially large habitat footprints such as mangroves) exhausted the website build’s JavaScript memory.  For now, you can refer to the lists above that detail the dive sites, fishing spots, archaeological sites, other special places, and habitats; spatial review of those layers is better done in the prep scripts.  If you are interested, we are happy to meet 1:1 and walk through this.  
...
if (!is.null(lsp_pa_map_sf) && nrow(lsp_pa_map_sf) > 0) {
  ...
  knitr::asis_output("*IN PROGRESS! (needs the dissolved GoC protected-area shapefile on Mazu at render time).*\n")
}
```
:::
...
This was then reviewed and edited by the Expert Working Group in our Meeting #2 in January, 2026, and confirmed by those experts asynchronously in April 2026.
