...
- **Número de empleos**: Empleos marinos escalados a población costera, comparados con un punto de referencia. Se muestran dos opciones para discusión entre expertos:
  ...
  - **Opción B (valor 2018):** valor de 2018 de cada región
...
**ECO** rastrea ingresos marinos (M000A de Censos Económicos), comparados con la Opción A (promedio histórico 2003–2013) o la Opción B (ingresos 2018).
...
le_plotly_ref_pair <- function(df_hist, df_2019, y_var, base_title, plot_fn = le_plotly_lines, y_range = NULL) {
  list(
    ...
    cen2019 = plot_fn(
      df_2019,
      y_var = y_var,
      title = paste0(base_title, " (2018 Reference)"),
      y_range = y_range
    )
  )
}
...
quality_plotly <- le_plotly_lines(
  ...
  y_var = "score",
  title = "LE:LIV: Quality of Jobs (misma referencia en ambos escenarios)"
)
...
Para **número de empleos** e **ingresos ECO**, las gráficas siguientes muestran dos opciones de referencia en figuras separadas (referencia histórica arriba, referencia 2018 abajo). **La calidad del empleo** usa el mismo cuantil 90 del Golfo en ambos escenarios. El **valor 2018** de la Opción B es el año de recolección 2018 de la publicación del Censo Económico 2019.

| Componente | Opción A: promedio histórico 2003–2013 | Opción B: valor 2018 |
|------------|----------------------------------------|----------------------|
| Número de empleos | Promedio de empleos per residente costero en 2003, 2008 y 2013 | Empleos per residente costero en 2018 |
| Ingresos ECO | Promedio de ingresos marinos en 2003, 2008 y 2013 | Ingresos marinos totales en 2018 |
...
**Cómo leer las gráficas:** Cada línea de color es una región OHI a lo largo de años censales (no anuales). La línea punteada en 100 es la meta de referencia. Las gráficas que dependen de la referencia de empleos o ingresos muestran **dos figuras separadas** para cada métrica (referencia histórica arriba, referencia 2018 abajo). Como los datos solo están disponibles en intervalos censales, las líneas conectan cinco puntos; cambios pronunciados entre puntos reflejan variaciones plurianuales, no volatilidad año a año.
...
**Qué mide:** El puntaje general de LE promediado entre todas las regiones OHI del Golfo en cada año censal.

**Fuente de datos:** Salidas integradas de Censos Económicos INEGI; el promedio del Golfo se calcula como la media de los puntajes regionales de LE en cada año censal, excluyendo la Región 2 por problemas de calidad de datos.
...
Las gráficas del Golfo resumen la trayectoria promedio entre regiones con datos suficientes. La gráfica de referencia histórica aparece arriba de la gráfica de referencia 2018.
...
**Procesamiento de datos:** Los empleos se suman entre sectores marinos, se ajustan por porción costera, se dividen entre población costera interpolada, y se comparan con el promedio per cápita 2003–2013 (Opción A) o el valor 2018 (Opción B). La calidad del empleo suma J300A y J400A per empleo (MXN constantes 2023) y se compara con el cuantil 90 del Golfo entre todas las regiones y años censales (sin cambios entre escenarios).
...
**Método (Opción B, valor 2018):** Empleos per población costera en año $t$ divididos entre el valor 2018 para esa región, limitado a 100.

$$
\text{Jobs}^{B}_{r,t} = \min\left(100,\ \frac{(J^{\text{marine}}_{r,t} / P^{\text{coastal}}_{r,t})}{(J^{\text{marine}}/P^{\text{coastal}})_{r,2018}} \times 100\right)
$$
...
**Método (Opción B — valor 2018):**

$$
\text{ECO}^{B}_{r,t} = \min\left(100,\ \frac{M000A_{r,t}}{M000A_{r,2018}} \times 100\right)
$$

::: {.callout-tip collapse="true"}
### Resultado de ECO

**Qué mide:** Ingresos operativos marinos respecto a la referencia regional (promedio histórico o valor 2018, según la figura).
...
Estas gráficas usan el archivo de actividad CE limpio del script 1, con los mismos ajustes de población costera aplicados en los scripts 2 a 4. **Empleos** son totales ajustados costeros (formales e informales) sumados en todas las regiones OHI. **Calidad del empleo** es pesos de prestaciones sociales per empleo en MXN constantes de 2023. **Ingresos** son ingresos operativos ajustados costeros en MXN constantes de 2023. Las líneas están coloreadas por sector marino.
...
sector_colors <- c(
  "Fishing"                        = "#0091FF",
  "Mariculture"                    = "#FF9E1B",
  "Seafood processing & trade"     = "#00C781",
  "Port & dock construction"       = "#B98B4E",
  "Boat building"                  = "#FF4FA0",
  "Tidal energy"                   = "#5FD4E3",
  "Oil & gas maritime transport"   = "#E8590C",
  "Cargo maritime transport"       = "#FFD500",
  "Marine tourist transport"       = "#7C6FF0",
  "Coastal recreation"             = "#9ED60B",
  "Travel planners"                = "#C061E8",
  "Accommodations & food/drink"    = "#FF2255",
  "Environmental research"         = "#A8A8A8"
)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height=2, fig.width=10, dpi=200, fig.bg='transparent'}
gg_sector_legend
```

Tres figuras separadas: empleos totales, calidad del empleo (prestaciones sociales per empleo) e ingresos totales, cada una sumada en todas las regiones OHI. Use la clave de colores arriba (o la leyenda junto a cada gráfica) para leer las líneas.
