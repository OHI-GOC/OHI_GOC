...
# Aguas Limpias EWG 3.3
**Sitio web interactivo:** [Aguas Limpias](https://ohi-goc.github.io/OHI_GOC/chapters/cw.html)
...
A nivel de la meta, combinamos los cuatro componentes con un promedio aritmético simple: CW = promedio(Eutrofización, Patógenos, Químicos, Residuos marinos)

$$\text{CW}_{r,t} = \frac{EUT_{r,t} + PAT_{r,t} + CHM_{r,t} + MAR_{r,t}}{4}$$
...
| Plásticos | Riesgo de artes de pesca abandonadas (“artes fantasma”). La basura de gran tamaño (*megatrash*) aún no ha sido puntuada. |
...
cw_score_plotly <- cw_plotly_lines(
  ...
  y_var = "final_score",
  title = "Aguas Limpias general (promedio aritmético de EUT, PAT, CHM, MAR)"
)
...
pest_plotly <- cw_plotly_lines(
  ...
  y_var = "final_score",
  title = "CW Químicos: Pesticides"
)

phrm_plotly <- cw_plotly_lines(
  add_rgn_labels(phrm_cs),
  y_var = "final_score",
  title = "CW Químicos: Pharmaceuticals"
)
...
Donde, en teoría, los Microplásticos se incluirían en la puntuación de Químicos, pero actualmente carecemos de datos confiables para microplásticos (ver sección 1.7.6). Algunas combinaciones de región y año tienen poca o ninguna dato aplicable para un subcomponente y se tratan como 100 en la puntuación combinada de Químicos (por ejemplo, plaguicidas y residuos peligrosos de la Región 1, y aguas residuales sin tratamiento de la Región 2).
...
*Fin del borrador en español para el Grupo de Trabajo de Expertos 3.3. Las gráficas aparecerán al renderizar este archivo Quarto en una máquina con acceso a las tablas de estatus de Aguas Limpias.*
