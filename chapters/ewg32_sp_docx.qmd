...
sp_plotly_lines <- function(df, y_var, title, y_range = c(0, 105)) {
  ...
  ggplot(df, aes(x = year, y = !!y_col, color = rgn_label, group = rgn_label)) +
...
sp_plotly_bars <- function(df, y_var, title, y_range = c(0, 105)) {
  ...
  ggplot(df, aes(x = rgn_label, y = !!y_col, fill = rgn_label)) +
    geom_col(show.legend = FALSE) +
    geom_hline(yintercept = 100, linetype = "dashed", color = "black", linewidth = 0.4) +
    scale_fill_manual(values = rgn_label_colors) +
    coord_cartesian(ylim = y_range) +
    labs(
      title = title,
      x = "Region",
...
# for the list, show presence by habitat type and OHI region only (not feature counts or area).
...
Overall CCE current status is shown for **2019-2025** only (see the note under Overall Sense of Place scores). The cultural places layer still plots through 2026 below so the group can see recent DENUE-based change without folding incomplete beach-adoption coverage into the composite.
...
3. Curate to cultural economic units with a meaningful coastal connection (see above for what is included and excluded).
...
ggplot(
  ...
  aes(x = year, y = culture_emp_per_local_pop, color = rgn_label, group = rgn_label)
) +
  geom_line(linewidth = 0.7) +
  geom_point(size = 1.5) +
  geom_hline(
    yintercept = cult_emp_refs$q75,
    linetype = "dashed",
    color = "#747428",
    linewidth = 0.6
  ) +
  geom_hline(
    yintercept = cult_emp_refs$q90,
    linetype = "dotted",
    color = "#C03729",
    linewidth = 0.6
  ) +
  ...
  scale_color_manual(values = rgn_label_colors, name = "Region") +
  coord_cartesian(
    ylim = c(0, cult_emp_refs$ymax * 1.05),
    clip = "off"
  ) +
  labs(
...
Below are the **cleaned** place lists used to calculate LSP current status (from the processed condition tables), grouped by type. These are preferred over the raw EWG inventory sheet, which still contains duplicates and superseded entries. Unique names in each group are listed below.
...
# static presence overview of special habitats by region
...
The interactive protected-area polygon map (colored by SIMEC i-efectividad / Ramsar management-plan condition) is available on the [Sense of Place EWG 3.2 website page](https://ohi-goc.github.io/OHI_GOC/chapters/ewg32_sp.html). It is not embedded in this Word document to keep the render lighter.

We do **not** host a separate full-polygon map for every LSP type on the website either. Embedding several detailed Leaflet layers (especially large habitat footprints such as mangroves) previously exhausted the website build’s JavaScript memory. The cleaned name lists above remain the inventory reference for dive sites, fishing spots, archaeological sites, other special places, and habitats; spatial review of those layers is better done in the prep scripts or GIS.
...
ggplot(lsp_cats, aes(x = rgn_label, y = condition, fill = category)) +
  ...
  labs(
    ...
    x = "Region",
    y = "Condition and Percent (out of 100)",
    fill = "LSP category"
  ) +
  gg_theme_pdf() +
  theme(axis.text.x = element_text(angle = 35, hjust = 1))
